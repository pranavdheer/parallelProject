...
int main(int argc, char **argv){
 ...
 int clustering, count, pseudo_listing, compact_forward, forward, edge_iterator, new_alg, nb_high;
 ...
 forward = compact_forward = edge_iterator = new_alg = 0;
 for (i=1; i<argc; i++){
 ...
 if (edge_iterator + forward + compact_forward + new_alg != 1)
  usage(argv[0]);
 ...
 fprintf(stderr," reading done in %ds...\n", (int)end);
 ...
 fprintf(stderr," renumbering and sorting done in %ds.\n", (int)end);
 fflush(stderr);
